...
class ViveTrackerModule():
    ...
    def update_add_device(self):
        poses = self.vr.getDeviceToAbsoluteTrackingPose(openvr.TrackingUniverseStanding, 0,
                                                               openvr.k_unMaxTrackedDeviceCount)
        for i in range(len(self.devices), openvr.k_unMaxTrackedDeviceCount):
            if poses[i].bDeviceIsConnected:
                self.add_tracked_device(i)
    ...
    def add_tracked_device(self, tracked_device_index):
        ...
        if (device_class == openvr.TrackedDeviceClass_Controller):
            device_keyword = "Controller"
        elif (device_class == openvr.TrackedDeviceClass_HMD):
            device_keyword = "HMD"
        elif (device_class == openvr.TrackedDeviceClass_GenericTracker):
            device_keyword = "Tracker"
        elif (device_class == openvr.TrackedDeviceClass_TrackingReference):
            device_keyword = "Tracking Reference"
        else:
            return AssertionError
        
        vr_device = vr_tracked_device(self.vr,i,device_keyword)
        if vr_device.get_serial() in self.device_serial_list:
            print("This device is already in list")
            return 
        device_name = f"{device_keyword}_"+str(len(self.object_names[device_keyword])+1)
        self.object_names[device_keyword].append(device_name)
        self.devices[device_name] = vr_device
        self.device_index_map[i] = device_name        
        self.device_serial_list.append(self.devices[device_name].get_serial())
